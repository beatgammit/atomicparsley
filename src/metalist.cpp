...
// same as the one above, but this doesn't put out any formatting, cuz JSON hates formatting
void APar_ExtractDataAtom_JSON(int this_atom_number) {
	if ( source_file != NULL ) {
		AtomicInfo* thisAtom = &parsedAtoms[this_atom_number];
		char* parent_atom_name;
		...
		if (thisAtom->AtomicLength > min_atom_datasize ) {
			...
			if (thisAtom->AtomicVerFlags == AtomFlags_Data_Text) {
				...
				if ( (memcmp(parent_atom_name, "trkn", 4) == 0) || (memcmp(parent_atom_name, "disk", 4) == 0) ) {
					if (UInt16FromBigEndian(data_payload+4) != 0) {
						fprintf(stdout, "%hu of %hu", UInt16FromBigEndian(data_payload+2), UInt16FromBigEndian(data_payload+4) );
					} else {
						fprintf(stdout, "%hu", UInt16FromBigEndian(data_payload+2) );
					}
					...
					if ( thisAtom->AtomicLength - atom_header_size < 3 ) { //oh, a 1byte int for genre number
						char* genre_string = GenreIntToString( UInt16FromBigEndian(data_payload) );
						if (genre_string != NULL) {
							fprintf(stdout,"%s", genre_string);
						} else {
							fprintf(stdout," out of bound value - %hu", UInt16FromBigEndian(data_payload) );
						}
					} else {
						fprintf(stdout," out of bound value - %hu", UInt16FromBigEndian(data_payload) );
					}
					...
					fprintf(stdout,"%s", data_payload);
						...
						switch(bytes_rep) {
							case 1 : {
								if ( (memcmp(parent_atom_name, "cpil", 4) == 0) || (memcmp(parent_atom_name, "pcst", 4) == 0) || (memcmp(parent_atom_name, "pgap", 4) == 0) ) {
									...
									stiks* returned_stik = MatchStikNumber((uint8_t)data_payload[0]);
									if (returned_stik != NULL) {
										fprintf(stdout, "%s", returned_stik->stik_string);
									} else {
										fprintf(stdout, "Unknown value: %" PRIu8, (uint8_t)data_payload[0]);
									}
									...
									fprintf(stdout, "%hhu", data_payload[0] );
								}
								break;
							}
							case 2 : { //tmpo
								fprintf(stdout, "%hu", UInt16FromBigEndian(data_payload) );
								break;
							}
							case 4 : { //tves, tvsn
								if (memcmp(parent_atom_name, "sfID", 4) == 0) {
									sfIDs* this_store = MatchStoreFrontNumber( UInt32FromBigEndian(data_payload) );
									if (this_store != NULL) {
										fprintf(stdout, "%s (%u)", this_store->storefront_string, this_store->storefront_number );
									} else {
										fprintf(stdout, "Unknown (%u)", UInt32FromBigEndian(data_payload) );
									}

								} else {
									fprintf(stdout, "%u", UInt32FromBigEndian(data_payload) );
								}
								break;
							}
							case 8 : {
								fprintf(stdout, "%" PRIu64, UInt64FromBigEndian(data_payload) );
								break;
							}
					}
						...
						fprintf(stdout, "FILE%s; mime-type=%s; description=%s", file_suffix, uuid_mimetype, uuid_description);
...
void APar_Print_iTunesData_JSON(const char *path, char* output_path, uint8_t supplemental_info, uint8_t target_information, AtomicInfo* ilstAtom) {
	printBOM();
            ...
            thisAtom->AtomicVerFlags == AtomFlags_Data_UInt) && target_information == PRINT_DATA ) {
				if (strncmp(parent->AtomicName, "----", 4) == 0) {
					if (memcmp(parsedAtoms[parent->AtomicNumber+2].AtomicName, "name", 4) == 0) {
						// make sure we put a comma at the end of each line and avoid putting one on the last line
						if(bFirst)
							bFirst = false;
						else
							fprintf(stdout, ",");

						fprintf(stdout, "\"%s [%s;%s]\":\"", parent->AtomicName, parsedAtoms[parent->AtomicNumber+1].ReverseDNSdomain, parsedAtoms[parent->AtomicNumber+2].ReverseDNSname);
						APar_ExtractDataAtom_JSON(i);
						// append last quote mark
						fprintf(stdout, "\"");
					}
...
//		if (artwork_count == 1) {
		fprintf(stdout, ",\"covr\":%i", artwork_count);
//		} else {
