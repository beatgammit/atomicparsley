...
int real_main(int argc, char *argv[])
{
	...
	while (1) {
		static struct option long_options[] = {
			...
			{ "json",				optional_argument,  NULL,		OPT_OutputJSON },
		...
		c = getopt_long(argc, argv, "mhTtEe:a:b:c:d:f:g:i:k:l:n:o:p:q::u:w:y:z:A:B:C:D:F:G:H:I:J:K:L:MN:QR:S:U:WXV:ZP 0xAA: 0xAB: 0xAC: 0xAD: 0xAE: 0xAF: 0xB0: 0xB1: 0xB2: 0xB3: 0xB4: 0xB5: 0xB6:", long_options, &option_index);
			...
			case OPT_OutputJSON:{
				if (argv[optind]) { //for utilities that write iTunes-style metadata into 3gp branded files
					...
					if (memcmp(argv[optind], "+", 1) == 0) {
						APar_Print_iTunesData_JSON(ISObasemediafile, NULL, PRINT_FREE_SPACE + PRINT_PADDING_SPACE + PRINT_USER_DATA_SPACE + PRINT_MEDIA_SPACE, PRINT_DATA );
					} else {
						fprintf(stdout, "---------------------------\n");
						...
						if (iTuneslistAtom != NULL) {
							...
							APar_Print_iTunesData_JSON(ISObasemediafile, NULL, PRINT_FREE_SPACE + PRINT_PADDING_SPACE + PRINT_USER_DATA_SPACE + PRINT_MEDIA_SPACE, PRINT_DATA, iTuneslistAtom );
						}
						fprintf(stdout, "---------------------------\n");
					}
					...
					if (metadata_style >= THIRD_GEN_PARTNER) {
						...
						APar_Print_iTunesData_JSON(ISObasemediafile, NULL, 0, PRINT_DATA); //false, don't try to extractPix
